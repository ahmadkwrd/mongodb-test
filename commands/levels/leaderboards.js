...
module.exports = class RankCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      aliases: ["leaderboards", "levels", "nivel", "niveles"],
  ...
  async run(msg) {
    ...
    if (count == 0)
      return msg.channel.send("No leaderboard data for this guild.");
    ...
    async function genData(page) {
      ...
      const data = await msg.client.levels.getLeaderboardOffset(
        msg.guild,
        page * 10
      );
      ...
      data.forEach(row => {
        ...
        output += `#${row.currentRank} - Nivel ${row.currentLevel}\n${row.name}\n`;
      });
    ...
    let embed = new MessageEmbed()
      .addField(
        `Leaderboard (Página ${page + 1}/${pages + 1})`,
        await genData(page)
      )
