...
module.exports = class SayCommand extends Command {
    ...
    async run(msg, {page}) {
        ...
        if (!serverQueue) {
        return msg.channel.send(":x: **No estoy escuchando música en este server!**")
        } else {
        const paginated = util.paginate(serverQueue.songs.slice(1), page, 10)
        let embed = new MessageEmbed()
        .setColor(config.defaultColor)
        .setTitle(`Añadido a la lista ${msg.guild.name}`)
        .setURL(`https://discord.gg/rApKP2J`)
          
        if(serverQueue.songs.length > 1) {
          embed.setDescription(stripIndents`
             __Escuchando ahora:__
          ...
          `)
          embed.setFooter(`Página ${paginated.page}/${paginated.maxPage}`)
        } else {
          embed.setDescription(stripIndents`
             __Escuchando ahora:__
             [${serverQueue.songs[0].title}](${serverQueue.songs[0].url}) | ${this.client.timeCalc(serverQueue.songs[0].duration)} | ${serverQueue.songs[0].requester}
